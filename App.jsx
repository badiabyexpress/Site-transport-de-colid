...
const PERMISSIONS_SCHEMA = [
  ...
  { group: "BORDEREAUX", permissions: [
    ...
    { key: "bordereaux.generer", label: "Générer un bordereau" },
  ]},
  { group: "PAIEMENTS & FACTURES", permissions: [
    { key: "paiements.voir_propres", label: "Voir ses propres paiements" },
    { key: "paiements.voir_tous", label: "Voir tous les paiements" },
    { key: "paiements.facture_pdf", label: "Télécharger une facture" },
  ]},
  ...
  { group: "CONFIGURATION", permissions: [
    ...
    { key: "config.tarifs", label: "Modifier les tarifs et routes" },
    { key: "config.categories", label: "Gérer les catégories" },
    { key: "config.utilisateurs", label: "Gérer les utilisateurs" },
  ]},
...
const ROLE_DEFAULT_PERMISSIONS = {
  ...
  "Agent": ["colis.voir_propres", "colis.voir_tous", "colis.creer", "colis.modifier", "colis.changer_statut", "colis.enregistrer_paiement", "bordereaux.consulter", "bordereaux.generer", "paiements.voir_propres", "clients.consulter", "ia.utiliser"],
  "Comptable": ["colis.voir_tous", "paiements.voir_propres", "paiements.voir_tous", "paiements.facture_pdf", "clients.consulter", "bordereaux.consulter"],
  "Chauffeur": ["colis.voir_propres", "colis.changer_statut"],
};
...
function App() {
  ...
  const nav = [
    ...
    { key: "colis", label: t.colis, icon: Package, show: canOps || canFinance || session.role === "Chauffeur" },
    { key: "clients", label: t.clients, icon: Users, show: canOps || canFinance },
    { key: "bordereaux", label: t.bordereaux, icon: FileStack, show: canOps },
    { key: "paiements", label: t.paiements, icon: Receipt, show: canOps || canFinance },
    { key: "comptabilite", label: "Comptabilité", icon: DollarSign, show: canFinance },
    { key: "ia", label: t.ia, icon: Sparkles, show: canOps },
    { key: "admin", label: t.admin, icon: Settings, show: canAdmin },
  ].filter((n) => n.show);
    ...
    <Shell rtl={rtl} theme={theme}>
      <div style={{ display: "flex", minHeight: "100vh", background: "var(--surface2)" }}>
        ...
        <main style={{ flex: 1, padding: "28px 32px", overflowY: "auto" }}>
          ...
          {view === "bordereaux" && <BordereauxPage data={data} />}
          ...
          {view === "admin" && canAdmin && <ConfigurationHub key={adminResetKey} data={data} persist={persist} session={session} notify={notify} onNavigateApp={setView} offline={offline} />}
        </main>
      </div>
...
function ColisView({ data, persist, session, notify, t }) {
  ...
  return (
    <div>
      ...
      {selected && <ColisDetail colis={selected} onClose={() => setSelected(null)} onAdvance={() => advance(selected.tracking)} onDelete={() => remove(selected.tracking)} onCancel={(motif) => annuler(selected.tracking, motif)} onUpdate={(patch) => updateColis(selected.tracking, patch)} canManage={!isChauffeur} isAdmin={session.role === "Administrateur"} isChauffeur={isChauffeur} />}
    </div>
  );
}

function BordereauxPage({ data }) {
  const [generating, setGenerating] = useState(null);
  const [error, setError] = useState(null);
  async function generate(country, direction) {
    const colisRoute = data.colis.filter((c) => c.pays === country.code && (c.direction || "export") === direction && c.status !== "Livré" && c.status !== "Annulé");
    if (colisRoute.length === 0) return;
    const key = `${country.code}-${direction}`;
    setGenerating(key); setError(null);
    try { await downloadRouteManifest(colisRoute, country, direction); }
    catch (e) { console.error(e); setError(key); }
    setGenerating(null);
  }
  return (
    <div>
      <h1 style={{ fontFamily: "'Space Grotesk',sans-serif", color: "var(--text)", fontSize: 24, margin: "0 0 4px" }}>Bordereaux d'envoi par route</h1>
      <p style={{ color: "var(--muted)", fontSize: 13.5, marginTop: 0, marginBottom: 22 }}>Chaque route existe dans les deux sens (export depuis Conakry et import vers Conakry). Le bordereau PDF regroupe tous les colis en attente (non livrés) de ce sens.</p>
      <div style={{ display: "flex", flexDirection: "column", gap: 12, maxWidth: 640 }}>
        {COUNTRIES.filter((c) => c.code !== "GN").map((c) => (
          <div key={c.code} style={{ background: "var(--surface)", borderRadius: 14, padding: "16px 18px", boxShadow: "0 2px 10px rgba(10,38,71,0.06)" }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)", marginBottom: 10 }}>{c.name} ({c.city})</div>
            {["export", "import"].map((direction) => {
              const count = data.colis.filter((x) => x.pays === c.code && (x.direction || "export") === direction && x.status !== "Livré" && x.status !== "Annulé").length;
              const key = `${c.code}-${direction}`;
              return (
                <div key={key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "var(--surface2)", borderRadius: 8, padding: "10px 14px", marginBottom: 8 }}>
                  <div>
                    <div style={{ fontSize: 12.5, color: "var(--text)", fontWeight: 600 }}>{direction === "export" ? `Conakry → ${c.city}` : `${c.city} → Conakry`}</div>
                    <div style={{ fontSize: 11, color: "var(--muted)" }}>{count} colis en attente</div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    {error === key && <span style={{ fontSize: 11, color: "#E23F52" }}>Échec</span>}
                    <button onClick={() => generate(c, direction)} disabled={count === 0 || generating === key} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 8, border: "none", background: count === 0 ? "var(--border)" : "#E23F52", color: "#fff", fontSize: 12.5, fontWeight: 700, cursor: count === 0 ? "not-allowed" : "pointer" }}>
                      <Download size={13} /> {generating === key ? "Génération…" : "PDF"}
                    </button>
                  </div>
                </div>
              );
...
function ColisForm({ onClose, onSave, existingColis, categories, session, sites }) {
  ...
  const [rabaisManuel, setRabaisManuel] = useState("0");
  ...
  const prixBrut = calcPrice(pays, poidsTotal, 0, mode);
  ...
  const discountManuel = Math.min(Math.max(Number(rabaisManuel) || 0, 0), 50);
  const discountTotal = Math.min(discountLoyalty + discountManuel, 60);
  const prix = +(prixBrut * (1 - discountTotal / 100)).toFixed(2);
  ...
  function submit(e) {
    ...
    onSave({
      ...
      prixBrut, discountLoyalty, discountManuel, discountTotal, prix, paye: payeNum, reste, photos,
        ...
        {step === 1 && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              ...
              <Field label="Téléphone *"><input value={expTelephone} onChange={(e) => setExpTelephone(e.target.value)} style={inputStyle} placeholder="+224 ### ###" /></Field>
        ...
        {step === 2 && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              ...
              <Field label="Téléphone *"><input value={destTelephone} onChange={(e) => setDestTelephone(e.target.value)} style={inputStyle} placeholder="Numéro WhatsApp" /></Field>
        ...
        {step === 4 && (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
              ...
              <Field label="Rabais additionnel (%)"><input value={rabaisManuel} onChange={(e) => setRabaisManuel(e.target.value)} style={inputStyle} placeholder="0" /></Field>
            ...
            <div style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 16px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "var(--text)", marginBottom: 6 }}><span>Frais de base ({poidsTotal.toFixed(1)} kg · {mode === "air" ? "Aérien" : "Maritime"})</span><span>{fmt(prixBrut, "GNF")}</span></div>
              {discountTotal > 0 && <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#3ECB84", marginBottom: 6 }}><span>Remise (-{discountTotal}%)</span><span>-{fmt(prixBrut - prix, "GNF")}</span></div>}
...
async function downloadRouteManifest(colisRoute, country, direction) {
  ...
  doc.text("Signature de l'agent :", 14, finalY + 16);
  doc.text("Signature du transporteur :", 110, finalY + 16);
...
async function downloadLabel(colis) {
  ...
  doc.setFillColor(10, 38, 71); doc.rect(0, 0, 100, 22, "F");
  doc.setTextColor(255, 255, 255); doc.setFontSize(13); doc.text("BA-DIABY EXPRESS", 6, 10);
  doc.setFontSize(8); doc.text(routeLabel(colis.pays, colis.direction), 6, 17);

  doc.setTextColor(20, 20, 20);
  doc.addImage(qrData, "PNG", 30, 28, 40, 40);

  doc.setFontSize(13); doc.setFont(undefined, "bold");
  doc.text(colis.tracking, 50, 76, { align: "center" });
  doc.setFont(undefined, "normal"); doc.setFontSize(9);

  let y = 86;
  const line = (label, value) => { doc.setTextColor(120, 120, 120); doc.text(label, 6, y); doc.setTextColor(20, 20, 20); doc.text(String(value), 6, y + 5); y += 12; };
  line("DESTINATAIRE", colis.destinataire);
  line("TÉLÉPHONE", colis.telephone);
  line("DESTINATION", `${dest?.name || ""} (${dest?.city || ""})`);
  line("POIDS / MODE", `${colis.poids} kg · ${colis.mode === "air" ? "Aérien" : "Maritime"}`);

  doc.setDrawColor(200); doc.line(6, 144, 94, 144);
  doc.setFontSize(7); doc.setTextColor(120, 120, 120);
  doc.text(`Édité le ${new Date().toLocaleDateString("fr-FR")} — ba-diaby-express.com`, 6, 148);
...
async function downloadInvoice(colis) {
  ...
  const rows = [
    ...
    ["Statut", colis.status],
    ...(colis.discountTotal ? [["Remise appliquée", `-${colis.discountTotal}% (brut ${fmt(colis.prixBrut || colis.prix, "EUR")})`]] : []),
...
function EditColisForm({ colis, onClose, onSave }) {
  ...
  const [rabaisManuel, setRabaisManuel] = useState(String(colis.discountManuel || 0));
  ...
  function submit(e) {
    ...
    onSave({
      ...
      prixBrut, discountLoyalty, discountManuel, discountTotal, prix, paye: payeNum, reste,
    });
  }
  ...
  return (
    <Modal onClose={onClose} title={`Modifier le colis ${colis.tracking}`} wide>
      <form onSubmit={submit} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        <Field label="Rabais additionnel (%)"><input value={rabaisManuel} onChange={(e) => setRabaisManuel(e.target.value)} style={inputStyle} /></Field>
...
function ColisDetail({ colis, onClose, onAdvance, onDelete, onCancel, onUpdate, canManage, isAdmin, isChauffeur }) {
  ...
  return (
    <Modal onClose={onClose} title="Détail du colis" wide>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 18 }}>
        ...
        <Info label="Reste à payer" value={fmt(colis.reste, "EUR")} accent={colis.reste > 0} />
        {colis.discountTotal > 0 && <Info label="Remise appliquée" value={`-${colis.discountTotal}% (prix brut ${fmt(colis.prixBrut || colis.prix, "EUR")})`} />}
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        <div style={{ display: "flex", gap: 14 }}>
          ...
          {isAdmin && <button onClick={onDelete} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", color: "#E23F52", fontSize: 13, cursor: "pointer" }}><Trash2 size={14} /> Supprimer</button>}
          {canManage && colis.status !== "Annulé" && colis.status !== "Livré" && <button onClick={() => setCancelling(true)} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", color: "#E0A63A", fontSize: 13, cursor: "pointer" }}><AlertTriangle size={14} /> Annuler le colis</button>}
        </div>
...
function Clients({ data }) {
  ...
  data.colis.forEach((c) => {
    if (!map[c.destinataire]) map[c.destinataire] = { nom: c.destinataire, telephone: c.telephone, count: 0, total: 0 };
    map[c.destinataire].count += 1; map[c.destinataire].total += c.prix;
  });
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--muted)", fontSize: 13.5, marginTop: 0, marginBottom: 22 }}>Historique et fidélité, générés à partir des expéditions enregistrées</p>
      <div style={{ background: "var(--surface)", borderRadius: 14, boxShadow: "0 2px 10px rgba(10,38,71,0.06)", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr style={{ background: "var(--surface2)", textAlign: "left" }}>{["Client", "Téléphone", "Expéditions", "Total dépensé", "Fidélité", "Remise applicable"].map((h) => <th key={h} style={{ padding: "12px 16px", fontSize: 11.5, color: "var(--muted)", fontWeight: 700 }}>{h}</th>)}</tr></thead>
          <tbody>
            {clients.map((c) => (
              <tr key={c.nom} style={{ borderTop: "1px solid var(--surface2)" }}>
                <td style={{ padding: "12px 16px", fontSize: 13.5, color: "var(--text)", fontWeight: 600 }}>{c.nom}</td>
                <td style={{ padding: "12px 16px", fontSize: 13, color: "var(--muted)" }}>{c.telephone}</td>
            ...
            {clients.length === 0 && <tr><td colSpan={6} style={{ padding: 20, color: "var(--muted)", fontSize: 13 }}>Aucun client pour le moment.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12, marginBottom: 18 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          {session?.role === "Administrateur" && <button onClick={() => setForm({ type: "Dépense", nom: "", montant: "", date: new Date().toISOString().slice(0,10) })} style={{ display: "flex", alignItems: "center", gap: 6, background: "#E23F52", color: "#fff", border: "none", borderRadius: 9, padding: "9px 14px", fontSize: 12.5, fontWeight: 700, cursor: "pointer" }}><Plus size={14} /> Ajouter</button>}
        </div>
      </div>
          ...
          <tbody>
            {depensesPeriode.map((d) => (
              <tr key={d.id} style={{ borderTop: "1px solid var(--border)" }}>
                ...
                <td style={{ padding: "12px 16px", textAlign: "right" }}>{session?.role === "Administrateur" && <button onClick={() => removeDepense(d.id)} style={{ background: "none", border: "none", color: "#E23F52", cursor: "pointer" }}><Trash2 size={14} /></button>}</td>
              </tr>
            ))}
...
{"expediteur_prenom": "", "expediteur_nom": "", "expediteur_telephone": "", "destinataire_prenom": "", "destinataire_nom": "", "destinataire_telephone": "", "pays_expediteur": "", "pays_destinataire": "", "poids_kg": 0, "prix_eur": null, "produit_nom": ""}`;
  ...
  function confirmer() {
    ...
    onCreate({
      ...
      prixBrut, discountLoyalty: 0, discountManuel: 0, discountTotal: 0, prix: prixBrut, paye: 0, reste: prixBrut, photos: [],
      status: "Enregistré", historique: [{ status: "Enregistré", date: new Date().toISOString() }],
      createdAt: new Date().toISOString(), pod: null, signature: null, driverLoc: null,
    });
    onClose();
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18, gap: 12 }}>
        ...
        <button onClick={() => setShowForm(true)} style={{ display: "flex", alignItems: "center", gap: 6, background: "#E23F52", color: "#fff", border: "none", borderRadius: 9, padding: "10px 16px", fontSize: 13.5, fontWeight: 700, cursor: "pointer", flexShrink: 0 }}><Plus size={16} /> Créer un compte</button>
      </div>
      <div style={{ background: "var(--surface)", borderRadius: 14, boxShadow: "0 2px 10px rgba(10,38,71,0.06)", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {data.users.map((u) => (
              <tr key={u.id} onClick={() => setEditingUser(u)} style={{ borderTop: "1px solid var(--surface2)", cursor: "pointer" }}>
                ...
                <td style={{ padding: "12px 16px", textAlign: "right" }} onClick={(e) => e.stopPropagation()}>{u.identifiant !== "admin" && <button onClick={() => removeUser(u.id)} style={{ background: "none", border: "none", color: "#E23F52", cursor: "pointer" }}><Trash2 size={15} /></button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
